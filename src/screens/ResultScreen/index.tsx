...
export default function ResultScreen() {
  ...
  async function handleShareFacebook() {
    ...
    setSharing('fb')
    try {
      const result = await shareToFacebook(shareCardRef.current, grade, score)
      if (result === 'downloaded') showToast('圖片已下載！請開啟 Facebook 並上傳此圖片')
  ...
  async function handleShareInstagram() {
    ...
    setSharing('ig')
    try {
      const result = await shareToInstagram(shareCardRef.current, grade, score)
      if (result === 'downloaded') showToast('圖片已下載！請開啟 Instagram 並上傳此圖片')
    } finally { setSharing(null) }
  }
